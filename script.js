let produtos = [];
   ...
   window.onload = () => {
     ...
     if (typeof window.db !== 'undefined' && window.db) carregarProdutosFirestore();
     mostrarSecao('painel');
   };
   ...
   function confirmarCriarProduto(status) {
     ...
     if (nome && quantidade >= 0 && categoria && valor >= 0 && typeof window.db !== 'undefined' && window.db) {
       const novoProduto = {
         nome,
         quantidade,
         categoria,
   ...
   function carregarProdutosFirestore() {
     if (!window.db || typeof window.db.collection !== 'function') {
       console.error("Firestore não inicializado corretamente:", window.db);
       alert("Erro: Firestore não está disponível. Verifique a console.");
       return;
     }
     window.db.collection("produtos").get().then((querySnapshot) => {
       produtos = [];
   ...
   function adicionarProdutoFirestore(produto) {
     if (!window.db) return;
     window.db.collection("produtos").add(produto).then((docRef) => {
   ...
   function atualizarProdutoFirestore(produto) {
     if (!window.db) return;
     window.db.collection("produtos").doc(produto.id).set(produto).then(() => {
   ...
   function excluirProduto(i, filtroStatus) {
     if (confirm("Deseja realmente excluir este produto?") && typeof window.db !== 'undefined' && window.db) {
       window.db.collection("produtos").doc(produtos[i].id).delete().then(() => {
         produtos.splice(i, 1);
         atualizarListas();
         atualizarDashboard();
